...
{
    public class AddInstaller : System.Configuration.Install.Installer
    {
        ...
        const int CSIDL_FLAG_PER_USER_INIT = 0x0800;        // combine with CSIDL_ value to indicate per-user 
        ...
        private const string Addin2005Path = @"Visual Studio 2005\Addins\CodeConvert";
        private const string Addin2008Path = @"Visual Studio 2008\Addins\CodeConvert";
        ...
        protected override void OnAfterInstall(System.Collections.IDictionary savedState)
        {
            ...
            try
            {
                string addinTargetPath2005 = Path.Combine(Environment.GetFolderPath(
                      Environment.SpecialFolder.MyDocuments),
                      Addin2005Path);

                string addinTargetPath2008 = Path.Combine(Environment.GetFolderPath(
                      Environment.SpecialFolder.MyDocuments),
                      Addin2008Path);
                ...
                if (!Directory.Exists(addinTargetPath2005))
                    Directory.CreateDirectory(addinTargetPath2005);

                if (!Directory.Exists(addinTargetPath2008))
                    Directory.CreateDirectory(addinTargetPath2008);

                string[] wholeFiles = Directory.GetFiles(assemblyPath);

                foreach (string tempFile in wholeFiles)
                {
                    File.Copy(tempFile, Path.Combine(addinTargetPath2005, Path.GetFileName(tempFile)));
                    File.Copy(tempFile, Path.Combine(addinTargetPath2008, Path.GetFileName(tempFile)));
                }
            }
            catch (Exception ex)
            {
                System.Windows.Forms.MessageBox.Show(ex.Message);
                System.Diagnostics.Debug.WriteLine("Error occurrred");
        ...
        public override void Uninstall(System.Collections.IDictionary savedState)
        {
            ...
            if (Directory.Exists(addinTargetPath2005))
                Directory.Delete(addinTargetPath2005,true);

            if (Directory.Exists(addinTargetPath2008))
                Directory.Delete(addinTargetPath2008,true);
        }
    }
}
