...
namespace ICSharpCode.NRefactory.PrettyPrinter
{
	public sealed class CSharpOutputVisitor : NodeTrackingAstVisitor, IOutputAstVisitor
	{
...
#else
            bool withinParentheses = this.prettyPrintOptions.WithinMethodDeclarationParentheses && (delegateDeclaration.Parameters != null && delegateDeclaration.Parameters.Count > 0);
#endif
		...
		void PrintMethodDeclaration(MethodDeclaration methodDeclaration)
		{
			...
			outputFormatter.PrintToken(Tokens.OpenParenthesis);
            bool withinParentheses = this.prettyPrintOptions.WithinMethodDeclarationParentheses && (methodDeclaration.Parameters != null && methodDeclaration.Parameters.Count > 0);//&& methodDeclaration.Parameters.Any ();
			if (withinParentheses) {
				outputFormatter.Space();
			}
		...
		public override object TrackedVisitConstructorDeclaration(ConstructorDeclaration constructorDeclaration, object data)
		{
			...
			outputFormatter.PrintToken(Tokens.OpenParenthesis);
            bool withinParentheses = this.prettyPrintOptions.WithinMethodDeclarationParentheses && (constructorDeclaration.Parameters != null && constructorDeclaration.Parameters.Count > 0);//&& constructorDeclaration.Parameters.Any ();
			if (withinParentheses) {
				outputFormatter.Space();
			}
	...
	//					outputFormatter.NewLine ();
		...
		public override object TrackedVisitAnonymousMethodExpression(AnonymousMethodExpression anonymousMethodExpression, object data)
		{
			...
			if (anonymousMethodExpression.Parameters.Count > 0 || anonymousMethodExpression.HasParameterList) {
				outputFormatter.PrintToken(Tokens.OpenParenthesis);
				bool withinParentheses = this.prettyPrintOptions.WithinMethodDeclarationParentheses && (anonymousMethodExpression.Parameters != null && anonymousMethodExpression.Parameters.Count > 0);
//&& anonymousMethodExpression.Parameters.Any ();
				if (withinParentheses) {
					outputFormatter.Space();
				}
